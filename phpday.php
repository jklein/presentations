<?php

$title = "PHPDay 2013 - High Performance PHP";
include 'includes/header.php';
?>

<div class="container">
    <div style="text-align:center">
        <iframe src="http://www.slideshare.net/slideshow/embed_code/20934134" width="683" height="570"
            frameborder="0" marginwidth="0" marginheight="0" scrolling="no"
            style="border:1px solid #CCC;border-width:1px 1px 0;margin-bottom:5px"
            allowfullscreen webkitallowfullscreen mozallowfullscreen>
        </iframe>
        <div style="margin-bottom:5px">
            <strong>
                <a href="http://www.slideshare.net/jnklein/phpday-2013-high-performance-php"
                    title="PHPDay 2013 - High Performance PHP" target="_blank">PHPDay 2013 - High Performance PHP
                </a>
            </strong> from
            <strong><a href="http://www.slideshare.net/jnklein" target="_blank">Jonathan Klein</a></strong>
        </div>
    <h4><a href="http://joind.in/talk/view/8644">Rate this talk!</a></h4>
    </div>

    <div class="resources row">
        <div class="span12">
            <h2>Resources</h2>
        </div>
        <div class="span6">
            <h5>The Value of Performance</h5>
            <ul>
                <li><a href="http://www.phpied.com/the-performance-business-pitch/">The Performance Business Pitch</a></li>
                <li><a href="http://torbit.com/blog/2012/09/19/some-interesting-performance-statistics/">Bounce Rate vs. Performance</a></li>
                <li><a href="http://www.stevesouders.com/blog/2012/02/10/the-performance-golden-rule/">The Performance Golden Rule</a></li>
                <li><a href="http://www.nngroup.com/articles/response-times-3-important-limits/">Response Time Limits</a></li>
            </ul>
            <h5>Monitoring Tools</h5>
            <ul>
                <li><a href="http://goo.gl/LbDPE">StatsD</a></li>
                <li><a href="http://graphite.wikidot.com/">Graphite</a></li>
                <li><a href="http://www.aosabook.org/en/graphite.html">Graphite Architecture</a></li>
                <li><a href="https://github.com/facebook/xhprof">xhprof</a></li>
                <li><a href="http://phpbench.com/">PHPBench</a></li>
            </ul>
            <h5>APC</h5>
            <ul>
                <li><a href="http://www.slideshare.net/vortexau/improving-php-application-performance-with-apc-presentation">Overview Presentation</a></li>
                <li><a href="http://engineering.wayfair.com/moving-constants-out-of-apc-and-into-cdb/">CDB vs. APC</a></li>
            </ul>
            <h5>Framework Benchmarks</h5>
            <ul>
                <li><a href="http://systemsarchitect.net/performance-benchmark-of-popular-php-frameworks/">PHP Framework Comparison</a></li>
                <li><a href="http://www.techempower.com/benchmarks/">Comparison of Many Web Frameworks</a></li>
            </ul>
        </div>
        <div class="span6">
            <h5>Upgrading PHP</h5>
            <ul>
                <li><a href="http://news.php.net/php.internals/36484">PHP 5.2 to 5.3 improvement</a></li>
                <li><a href="http://news.php.net/php.internals/57760">PHP 5.3 to 5.4 improvement</a></li>
                <li><a href="http://php.net/migration53">PHP 5.2 to 5.3 migration guide</a></li>
                <li><a href="http://php.net/migration54">PHP 5.3 to 5.4 migration guide</a></li>
                <li><a href="http://static.zend.com/topics/White-paper-PHP4-PHP5.pdf">PHP5 vs. PHP4</a></li>
            </ul>
            <h5>HipHop</h5>
            <ul>
                <li><a href="http://developers.facebook.com/blog/post/2010/02/02/hiphop-for-php--move-fast/">Announcement</a></li>
                <li><a href="https://www.facebook.com/notes/facebook-engineering/speeding-up-php-based-development-with-hiphop-vm/10151170460698920">HipHop VM</a></li>
                <li><a href="https://github.com/facebook/hiphop-php">Github project</a></li>
            </ul>
            <h5>Other Links</h5>
            <ul>
                <li><a href="http://code.flickr.net/2009/12/02/flipping-out/">Feature Flags at Flickr</a></li>
                <li><a href="http://web-performance.meetup.com/">Web Performance Meetups</a></li>
                <li><a href="http://www.etsy.com/careers">Etsy Careers</a></li>
                <li><a href="http://www.jonathanklein.net">My Blog</a></li>
            </ul>
        </div>
    </div>
</div>
<?php

include 'includes/footer.php';
